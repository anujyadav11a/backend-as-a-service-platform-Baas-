...
import { COLLECTION_VALIDATION } from '../constants/validation';

export function CreateCollectionModal({ isOpen, onClose, onSubmit, loading }) {
  ...
  const onFormSubmit = async (data) => {
    ...
    if (result.success) {
      ...
      setSubmitError(result.error || 'Failed to create collection');
    }
  };
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between mb-4">
            <h2 id="create-modal-title" className="text-lg font-semibold text-gray-900">Create New Collection</h2>
          ...
          <form onSubmit={handleFormSubmit(onFormSubmit)} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <input
                ...
                {...register('name', COLLECTION_VALIDATION.name)}
            ...
            <div>
              ...
              <textarea
                ...
                {...register('description', COLLECTION_VALIDATION.description)}
                className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                  errors.description ? 'border-red-300' : 'border-gray-300'
                }`}
                placeholder="Collection for user data"
                rows={3}
                disabled={loading}
              />
              {errors.description && (
